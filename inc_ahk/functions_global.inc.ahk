...
toolTipGui(text, x := 0, y := 0, hiddenTitle := "", bgColor := "", fontSize := 0){
	...
	if(!regExMatch(y,"^\d")){
		...
		if(yTemp=="|"){
			...
			SysGet, Mon_, Monitor,0 ; MsgBox, Left: %Mon_Left% -- Top: %Mon_Top% -- Right: %Mon_Right% -- Bottom %Mon_Bottom%.
			x := -155 + Mon_Left + Ceil(( Mon_Right - Mon_Left ) / 4 * 3)
			yTemp := substr(y,2)
		}
...
DynaRun(TempScript, pipename=""){
	...
	if(false && InStr(A_ComputerName,"SL5") )
		ToolTip9sec( "`n(" A_ThisFunc " " A_ScriptName ">" RegExReplace(A_LineFile,".*\\") ":"  A_LineNumber ")" )
