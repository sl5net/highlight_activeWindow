...
; taskbarArea := { left:0, rigth:0, top:0, bottom:0, width:0 , height:0 }
		...
		; MsgBox, % A_ScreenWidth
			if(X < 300 ){
				...
				if(X >= 28 ){
					...
					layout := "|1"
					; >= 28 it top, left else about 6
					yTaskBarBorder :=  Y+H//2 ; doesent matter. set middle
					xTaskBarBorder := 2 * ((X+W//2)) - 1 
					taskbarArea["rigth"] := xTaskBarBorder ; { left:0, rigth:0, top:0, bottom:0, width:0 , height:0 }				
					taskbarArea["w"] := xTaskBarBorder ; { left:0, rigth:0, top:0, bottom:0, width:0 , height:0 }				
					taskbarArea["h"] := A_ScreenHeight
					taskbarArea["bottom"] := A_ScreenHeight
				}else{
					; on first monitor top
					taskbarArea["taskbarMonitorNum"] := 1
					layout := "¯1"
					xTaskBarBorder := X+W//2 ; doesent matter. set middle
					yTaskBarBorder := 2 * (Y+H//2) + 1
					taskbarArea["x"] := 1
					taskbarArea["w"] := A_ScreenWidth
					taskbarArea["right"] := A_ScreenWidth
					taskbarArea["h"] := yTaskBarBorder
					taskbarArea["bottom"] := yTaskBarBorder
				}
			; MsgBox, % X ; >= 28 it top, left else about 6
			}else if(X > A_ScreenWidth - 300 && X < A_ScreenWidth ){
				; on first monitor top,right
				taskbarArea["taskbarMonitorNum"] := 1
				layout := "1|"
				xTaskBarBorder := A_ScreenWidth - 2 * (A_ScreenWidth - (X+W//2)) + 1 
				yTaskBarBorder :=  Y+H//2 ; doesent matter. set middle
				taskbarArea["x"] := xTaskBarBorder
				taskbarArea["w"] := A_ScreenWidth  - xTaskBarBorder
				taskbarArea["h"] := A_ScreenHeight
				taskbarArea["bottom"] := A_ScreenHeight
				taskbarArea["right"] := A_ScreenWidth
			}else if(X > A_ScreenWidth && X < A_ScreenWidth + 300 ){
				if(X >= A_ScreenWidth + 28 ){
					; on second monitor left
					taskbarArea["taskbarMonitorNum"] := 2
					layout := "|2"
					xTaskBarBorder := A_ScreenWidth + 2 * ( - A_ScreenWidth + (X+W//2)) - 1 
					yTaskBarBorder :=  Y+H//2 ; doesent matter. set middle
					taskbarArea["x"] := A_ScreenWidth 
					taskbarArea["y"] := 1
					taskbarArea["h"] := A_ScreenHeight
					taskbarArea["w"] := xTaskBarBorder - A_ScreenWidth 
					taskbarArea["right"] := xTaskBarBorder
					taskbarArea["bottom"] := A_ScreenHeight
				}else{
					...
					taskbarArea["h"] := yTaskBarBorder
					taskbarArea["bottom"] := yTaskBarBorder
					taskbarArea["right"] := 2*A_ScreenWidth
				}
			}else if(X > 2*A_ScreenWidth - 300 && X < 2*A_ScreenWidth ){
				; on second monitor top,rigth
				layout := "2|"
				taskbarArea["taskbarMonitorNum"] := 2
				xTaskBarBorder := 2*A_ScreenWidth - 2 * (2*A_ScreenWidth - (X+W//2)) + 1 
				yTaskBarBorder :=  Y+H//2 ; doesent matter. set middle
				...
				taskbarArea["w"] := 2*A_ScreenWidth - xTaskBarBorder 
				taskbarArea["right"] := 2*A_ScreenWidth
				taskbarArea["bottom"] := A_ScreenHeight
			}else{
				MsgBox,never happens ? 19-03-14_10-07
			; taskbar its at the top
				xTaskBarBorder := X+W//2 ; doesent matter. set middle
				yTaskBarBorder := 2 * (Y+H//2) + 1 
			}
		} else if(y > A_ScreenHeight - 300){
			xTaskBarBorder := X+W//2
			yTaskBarBorder := A_ScreenHeight - 2 * (A_ScreenHeight-(Y+H//2)) + 3 
		; MouseMove, X+W//2, Y+H//2 ; centere of icon
		; MsgBox, % H " ? " A_ScreenHeight 
			if(X > A_ScreenWidth ){
				; taskbar is second monitor at bottom
				taskbarArea["taskbarMonitorNum"] := 2
				layout := "_2"
				taskbarArea["x"] := A_ScreenWidth
				taskbarArea["y"] := yTaskBarBorder
				taskbarArea["h"] := A_ScreenHeight - yTaskBarBorder
				taskbarArea["w"] := A_ScreenWidth
				taskbarArea["right"] := 2*A_ScreenWidth
				taskbarArea["bottom"] := A_ScreenHeight
			}else{
	...
	if(doShowArea){
		MouseMove, taskbarArea["x"], taskbarArea["y"], 10
		MouseMove, taskbarArea["right"], taskbarArea["y"], 10
		MouseMove, taskbarArea["right"], taskbarArea["bottom"], 10
		MouseMove, taskbarArea["x"], taskbarArea["bottom"], 10
		MouseMove, taskbarArea["x"], taskbarArea["y"], 10	
	}
	return taskbarArea
}
;\____ taskbarArea __ 190314102208 __ 14.03.2019 10:22:08 __/
